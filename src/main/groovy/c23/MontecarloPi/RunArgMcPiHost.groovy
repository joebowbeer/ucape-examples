...
assert "Expected args = [workers, cores, iterations]" && (3 == args.size())
def workers = args[0] as int
def cores = args[1] as int
def iterations = args[2] as int
...
for ( w in 0..< workers) {
  workerObjects << new WorkerObject (
    workerProcess: new McPiWorker(cores: cores),
    inConnections : [100],
