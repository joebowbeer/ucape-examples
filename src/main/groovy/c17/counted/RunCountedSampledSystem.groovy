...
def sampler = new CountingSampler (
  inChannel: a.in(),
  outChannel: b.out(),
  sampleRequest: e.in(),
  countReturn: g.out() )

def samplingTimer = new CountedSamplingTimer (
  sampleRequest: e.out(),
  sampleInterval: 2500,
  countReturn: g.in(),
  countToGatherer: h.out() )

def sampledNetwork = new CountedSampledNetwork (
  inChannel: b.in(),
  outChannel: c.out() )

def gatherer = new CountingGatherer (
  inChannel: c.in(),
  outChannel: d.out(),
  gatheredData: f.out(),
  countInput: h.in() )
...
def printResults = new GPrint (
  inChannel: d.in(),
  heading: "output Values",
  delay: 0)

def network = [
  dataGen, sampler, samplingTimer, sampledNetwork, gatherer, evaluator, printResults
]
