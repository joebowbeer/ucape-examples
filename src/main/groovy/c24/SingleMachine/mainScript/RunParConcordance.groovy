...
import c24.SingleMachine.supportProcesses.parExtractConcordance
import c24.SingleMachine.supportProcesses.parFindEqualKeys
import c24.SingleMachine.supportProcesses.parSequencer

def inRoot = '/'
def outRoot = '/Concordance/OutputFiles/Parallel/'
...
//getClass().getClassLoader().getURLs().each { url->
...
for (source in ["ACM", "TMM", "WAD", "bible", "2bibles", "4bibles"]){
  ...
  for (run in 1..runs){
    ...
    def procList1 = (2..N).collect {n ->
      return new parSequencer(
        n: n,
        inList: NSequenceLists[1],
    ...
    def procList3 = (1..N).collect { n ->
      return new parExtractConcordance (
        equalMap: equalKeyMapList[n],
        n: n,
        startIndex: 0,
        words: wordBuffer,
        minSeqLen: minSeqLen,
