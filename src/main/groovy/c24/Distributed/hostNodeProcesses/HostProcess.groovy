...
import c24.Distributed.loadedProcesses.LoadedMerger
import c24.Distributed.loadedProcesses.LoadedNode
import c24.Distributed.loadedProcesses.LoadedReader
...
def inRoot = '/'
def outRoot = '/Concordance/OutputFiles/Distributed/'
def timeRoot = '/Concordance/OutputFiles/Distributed/Times/'
new File(outRoot).mkdirs()
new File(timeRoot).mkdirs()
...
//def minSeqLen = Ask.Int("Minimum number of sequence repetitions? (1 upwards) ", 1, 100)
//def blockLength = Ask.Int("blocksize? (min 2*N, max 10000) ", 2*N, 10000)
def runId = showInputDialog('HOST: What is the run identifier for timing purposes?')
def N = showInputDialog('What is the maximum number of words (N) in a sequence (1..100)?') as int
def nodes = showInputDialog('How many worker nodes (1..12)?') as int
def minSeqLen = showInputDialog('Minimum number of sequence repetitions (1..100)?') as int
def blockLength = showInputDialog('blocksize (2*N..10000)?') as int

println "Possible sources: ACM  TMM  WaD  bible  2bibles  4bibles"
//def nSources = Ask.Int ("Number of source texts? (1,10) ", 1, 10)
def nSources = showInputDialog('Number of source texts (1..10)?') as int
def sourceList = []
for ( s in 1..nSources){
  print " source $s "
//  sourceList << Ask.string("FileName omitting extension? ")
  sourceList << showInputDialog('FileName omitting extension?')
}
//def runs = Ask.Int("How many runs per text (>=1) ", 1, 100)
def runs = showInputDialog('How many runs per text (1..100)?') as int
...
for ( w in 1 .. nodes) {
  workerObjects << new WorkerObject (
    workerProcess: new LoadedNode(
      N: N,
      runs: runs,
      node: w,
...
def mergerObjects = []
for ( m in 1 .. N) {
  mergerObjects << new WorkerObject (
    workerProcess: new LoadedMerger(
      N: m,
      runs: runs,
      sourceList: sourceList,
      minSeqLen: minSeqLen,
